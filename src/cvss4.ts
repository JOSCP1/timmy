const CVSS4: CVSS4Module = (() => {
  ...
  function score(m: Partial<CVSSMetrics> = {}): number {
    const v = { ...DEFAULTS, ...m };
    const expl = 8.22
    ...
    return parseFloat(Math.min(10, Math.min(1,expl) * Math.min(1,iVS+iSS) * 10).toFixed(1));
  }

  function qualitative(s: number): CVSSQualitative {
  ...
  function vector(m: Partial<CVSSMetrics> = {}): string {
    const v = { ...DEFAULTS, ...m };
    return `CVSS:4.0/AV:${v.AV}/AC:${v.AC}/AT:${v.AT}/PR:${v.PR}/UI:${v.UI}/VC:${v.VC}/VI:${v.VI}/VA:${v.VA}/SC:${v.SC}/SI:${v.SI}/SA:${v.SA}`;
  }

  function metricsHTML(vid: string, m: Partial<CVSSMetrics> = {}): string {
    const v = { ...DEFAULTS, ...m };
    const sel = (name: keyof CVSSMetrics, opts: [string,string][]): string => {
  ...
  function updateDisplay(vid: string, m: Partial<CVSSMetrics>): void {
    const s = score(m), q = qualitative(s);
    ...
    if (el) { el.textContent = String(s); el.className = `score-value ${q.cls}`; }
