const Assets: AssetsModule = (() => {
  ...
  function getOrdered(): AssetRecord[] {
    ...
    return assetOrder.map(id => all.find(a => a.id === id)).filter((a): a is AssetRecord => !!a);
  }
  ...
  function refresh(): void {
    ...
    const tbody  = document.getElementById('assetsTbody') as HTMLTableSectionElement | null;
    const empty  = document.getElementById('assetsEmpty');
    const badge  = document.getElementById('assetsBadge');
    const search = ((document.getElementById('assetSearch') as HTMLInputElement|null)?.value || '').toLowerCase();
    if (badge) badge.textContent = String(assets.length);
  ...
  function filter(val: string): void { filterType = val; refresh(); }
  function search(_q: string): void  { refresh(); }

  function exportCSV(): void {
    const assets = getOrdered();
    const vulns  = VulnMgmt.getAll();
    const rows: string[][] = [['ID','Name','Type','C','I','A','Justification','Threats']];
    assets.forEach(a => {
      rows.push([
        a.tmId||a.id, `"${(a.name||'').replace(/"/g,'""')}"`, a.type,
        a.cia?.c||'N', a.cia?.i||'N', a.cia?.a||'N',
        `"${(a.justification||'').replace(/"/g,'""')}"`,
        String(vulns.filter(v => v.assetId === a.id).length),
      ]);
    });
    const blob = new Blob([rows.map(r => r.join(',')).join('\n')], { type:'text/csv' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob); link.download = 'assets.csv';
    link.click(); URL.revokeObjectURL(link.href);
    App.toast('Assets exported as CSV.', 'ok');
  }

  function getOrder(): string[]        { return assetOrder; }
  function setOrder(arr: string[]): void { assetOrder = arr || []; }

  function typeChip(type: string): string {
    const m: Record<string,string> = {
      process:'chip-process Process', store:'chip-store Store',
  ...
  function ciaChip(v: string | undefined): string {
    const m: Record<string,string> = { H:'chip-h High', M:'chip-m Med', L:'chip-l Low', N:'chip-n None' };
    const [c,l] = (m[v||'N']||'chip-n None').split(' ');
    return `<span class="chip ${c}">${l}</span>`;
  }
